...
describe('The copy method', () => {
  describe('should copy specified files with globs:', () => {
    ...
    it('lib async version.', done => {
      cpx.copy('test-ws/a/**/*.txt', 'test-ws/b', () => verifyFiles().then(() => done(), done));
    });

    it('lib async version (promise).', () => cpx.copy('test-ws/a/**/*.txt', 'test-ws/b').then(verifyFiles));
  ...
  describe('should clean and copy specified files with globs when give clean option:', () => {
    ...
    it('lib async version.', done => {
      cpx.copy('test-ws/a/**/*.txt', 'test-ws/b', { clean: true }, () => verifyFiles().then(() => done(), done));
    });
  ...
  describe('should copy files inside of symlink directory when `--dereference` option was specified:', () => {
    beforeEach(
      co.wrap(function*() {
        ...
        yield fs.symlink(path.resolve('test-ws/src'), path.resolve('test-ws/a/link'), 'junction');
      })
    );
    afterEach(() => teardownTestDir('test-ws'));
    ...
    it('lib async version.', done => {
      cpx.copy('test-ws/a/**/*.txt', 'test-ws/b', { dereference: true }, () => verifyFiles().then(() => done(), done));
    });
  ...
  describe('should copy specified files with globs when `--preserve` option was given:', () => {
    ...
    it('lib async version.', done => {
      cpx.copy('test-ws/a/**/*.txt', 'test-ws/b', { preserve: true }, () => verifyFiles().then(() => done(), done));
    });
